package gov.nysed.oce.ldgrants.grants.grant.service;

import java.util.ArrayList;
import java.util.List;

import gov.nysed.oce.ldgrants.grants.grant.dao.AuthorizationDao;
import gov.nysed.oce.ldgrants.grants.grant.domain.Authorization;
import gov.nysed.oce.ldgrants.user.domain.User;

public class AuthorizationService implements GenericDaoService<Authorization, Long> {

	
	AuthorizationDao dao = new AuthorizationDao();
	@Override
	public Long insert(Authorization t, User user) {
		
		return dao.insert(t, user);
	}

	@Override
	public Long update(Authorization t, User user) {
		
		return dao.update(t, user);
	}

	@Override
	public boolean delete(Long id) {
		
		return dao.delete(id);
	}

	@Override
	public Authorization select(Long id) {
		
		return dao.select(id);
	}

	@Override
	public List<Authorization> selectAll() {
		
		return dao.selectAll();
	}
	
	public ArrayList<Authorization> searchByTitle (String title){
		
		return dao.searchByTitle(title);
	}

}
